...
func TestRotateRnon0(t *testing.T) {
	p1 := NewPoint(10.5, 6.9)
	p2 := NewPoint(4, 1)
	r := NewRectangle(p1, p2)
	fmt.Println(r)
	...
	fmt.Println(r)
	//((4,-5.5) (9.9,1))
	if !intorno(r.pLL.x, 4) || !intorno(r.pLL.y, -5.5) || !intorno(r.pUR.x, 9.9) || !intorno(r.pUR.y, 1) {
		t.FailNow()
	}
}
...
func TestString(t *testing.T) {
	...
	p2 := NewPoint(4.1, 0.1)
	r := NewRectangle(p1, p2)
	fmt.Println("test string:", r)
	s := r.String()
	if !(strings.Contains(s, "4.1") &&
		strings.Contains(s, "6.9") &&
		strings.Contains(s, "10.5") &&
		strings.Contains(s, "6.9") &&
		strings.Contains(s, "((") &&
		strings.Contains(s, "))")) {
		t.FailNow()
