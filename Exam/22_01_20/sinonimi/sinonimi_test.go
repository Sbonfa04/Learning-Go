...
In caso di mancanza di uno dei due parametri il programma dovrà stampare "2 file names, please" e terminare.
In caso di assenza di uno dei due file (i cui nomi sono stati passati a linea di comando) il programma dovrà stampare rispettivamente "file 1 not found" e "file 2 not found" e terminare.

L'elaborazione consiste nel produrre su standard output un nuovo testo in cui tutte le parole che possiedono un sinonimo (secondo il file dei sinonimi) vengono arricchite dei loro sinonimi racchiusi tra parentesi quadre e in ordine alfabetico, ad esempio:
