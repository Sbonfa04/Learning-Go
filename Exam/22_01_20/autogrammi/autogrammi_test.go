...
func TestCalcNorm(t *testing.T) {
	...
	if q != 3 || min != 3 || max != 3 {
		fmt.Println("Calc1", q, min, max)
		t.FailNow()
	}

	//2
	q, min, max = CalcQuanteMinMax("uno, 2, quattro,")
	if q != 2 || min != 3 || max != 7 {
		fmt.Println("Calc2", q, min, max)
		t.FailNow()
	}
	...
	if q != 4 || min != 2 || max != 7 {
		fmt.Println("Calc3", q, min, max)

		t.FailNow()
	}

	//4
	q, min, max = CalcQuanteMinMax("conta se sono 10 le parole presenti nella stringa, la punteggiatura non fa parte della parola da contare")
	if q != 17 || min != 2 || max != 13 {
		fmt.Println("Calc4", q, min, max)
		t.FailNow()
	}

	//5
	q, min, max = CalcQuanteMinMax("1 punteggiatura non fa parte della parola da contare")
	if q != 8 || min != 2 || max != 13 {
		fmt.Println("Calc5", q, min, max)
		t.FailNow()
	}
...
func TestTrovaNumParoleInFrase(t *testing.T) {
	//1
	num1 := TrovaNumDopoKeyword("questa lunghissima frase invece contiene: 17 parole, lunghezza massima: 11, lunghezza minima: 2, esclusi numeri si intende", "contiene:")
	if num1 != 17 {
		fmt.Println("TrovaNum1", num1)
		t.FailNow()
	}

	//2
	num2 := TrovaNumDopoKeyword("questa contiene: 5 parole, ...", "contiene:")
	if num2 != 5 {
		fmt.Println("TrovaNum2", num2)

		t.FailNow()
	}

	//3
	num3 := TrovaNumDopoKeyword("le parole in questa frase in italiano sono == 8", "==")
	if num3 != 8 {
		fmt.Println("TrovaNum3", num3)
		t.FailNow()
	}
